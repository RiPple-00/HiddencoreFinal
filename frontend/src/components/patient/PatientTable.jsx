...
export default function PatientTable({ patients, onRowClick }) {
  ...
  const startNumber = totalCount === 0 ? 0 : (currentPage - 1) * ROWS_PER_PAGE + 1;
  ...
  const getStatusLabel = (status) => {
    const key = `patient.status.${(status || '').toLowerCase()}`;
    return t(key, '-');
  };
    ...
    <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white">
      <table className="w-full border-collapse bg-white">
        <thead className="bg-slate-100 text-sm font-semibold text-slate-500">
          <tr>
            <th className="px-8 py-5 text-center">{t('patient.table.id')}</th>
            <th className="px-8 py-5 text-center">{t('patient.table.name')}</th>
            <th className="px-8 py-5 text-center">{t('patient.table.genderAge')}</th>
            <th className="px-8 py-5 text-center">{t('patient.table.ward')}</th>
            <th className="px-8 py-5 text-center">{t('patient.table.admissionDate')}</th>
            <th className="px-8 py-5 text-center">{t('patient.table.status')}</th>
          </tr>
        </thead>
            ...
            currentPatients.map((patient) => (
              ...
              >
                ...
                <td className="px-8 py-7 text-center text-[18px] text-slate-700">
                  {getGenderLabel(patient.gender)} / {patient.age}
                </td>
                <td className="px-8 py-7 text-center text-[18px] text-slate-700">
                ...
                <td className="px-8 py-7 text-center">
                  <span
                    className={`inline-flex rounded-full px-4 py-2 text-sm font-semibold ${getStatusClassName(patient.patientStatus)}`}
                  >
      ...
      {totalCount > 0 && (
        <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-8 py-5">
          <p className="text-sm text-slate-500">
            {t('patient.table.showingOf', { total: totalCount.toLocaleString(), from: startNumber, to: endNumber })}
          </p>
