  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between gap-3 min-w-0">
        <span className="text-sm text-gray-700 shrink-0">{t('board.create.panel.pin', '상단 고정')}</span>
        <button
          type="button"
          aria-pressed={isPinned}
          aria-label={t('board.create.panel.pin', '상단 고정')}
      ...
      {/* 게시 상태 */}
      <div>
        <p className="text-sm font-medium text-gray-700 mb-2">{t('board.create.panel.publish', '게시 상태')}</p>
        <div className="flex flex-col gap-2">
          ...
          `}>
            ...
            />
            <div>
              <p className="text-sm font-medium text-gray-800">{t('board.create.panel.immediate', '즉시 게시')}</p>
              <p className="text-xs text-gray-400">{t('board.create.panel.immediateDesc', '저장 후 바로 시스템에 노출됩니다.')}</p>
            </div>
          </label>
          ...
          `}>
            ...
            />
            <div>
              <p className="text-sm font-medium text-gray-800">{t('board.create.panel.scheduled', '예약 게시')}</p>
              <p className="text-xs text-gray-400">{t('board.create.panel.scheduledDesc', '지정한 일시에 자동으로 게시됩니다.')}</p>
            </div>
          </label>
  ...
  return (
    <div className="flex flex-col gap-1 text-sm">
      <div className="flex justify-between">
        <span className="text-gray-400">{t('board.create.panel.author', '작성자')}</span>
        {/* CHECK!!! AuthContext 연동 후 실제 user?.name으로 교체 */}
        <span className="text-gray-700 font-medium">{authorName ?? '-'}</span>
      </div>
      <div className="flex justify-between">
        <span className="text-gray-400">{t('board.create.panel.lastUpdated', '마지막 수정')}</span>
...
export const PanelActions = ({ onSaveDraft, onSubmit, onCancel, isSubmitting }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        {t('board.create.panel.saveDraft', '임시 저장')}
      </Button>
      <Button
        type="button"
        variant="primary"
        ...
        className="w-full !rounded-lg"
      >
        {isSubmitting ? t('board.create.panel.submitting', '등록 중...') : t('board.create.panel.submit', '✓ 게시글 등록')}
      </Button>
      <Button
        type="button"
        variant="secondary"
        ...
        className="w-full !rounded-lg"
      >
        {t('board.create.panel.cancel', '취소')}
      </Button>
    </div>
  );
};
