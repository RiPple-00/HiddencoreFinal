...
export default function PatientListPage() {
  ...
  const summary = useMemo(() => { //상단에 요약 카드 계산 로직
    ...
    return {
      admittedCount: patientList.filter( //전체 환자 수
        (patient) => patient.patientStatus !== "DISCHARGED"
      ).length,

      dischargeSoonCount: patientList.filter( //퇴원 대기
        (patient) => patient.patientStatus === "DISCHARGE_SOON"
      ).length,

      unassignedCount: patientList.filter(//입원 대기
        (patient) => (!patient.building || !patient.room) && patient.patientStatus !== "DISCHARGED"
      ).length
    };
  }, [patients]);
    ...
    return (Array.isArray(patients) ? patients : []).filter((patient) => {
      ...
      const matchesQuickFilter =
        ...
        (quickFilter === "UNASSIGNED" &&
          (!patient.building || !patient.room) && patient.patientStatus !== "DISCHARGED");
